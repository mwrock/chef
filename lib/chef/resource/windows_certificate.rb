...
class Chef
      ...
      **Remove all certificates matching the subject**
      ...
      action_class do
        ...
        def export_cert(cert_obj, cert_path)
          ...
          when ".cer"
            cert_out = powershell_exec("openssl x509 -text -inform DER -in #{cert_obj.to_pem} -outform CER").result
            out_file.puts(cert_out)
          when ".crt"
            cert_out = powershell_exec("openssl x509 -text -inform DER -in #{cert_obj.to_pem} -outform CRT").result
            out_file.puts(cert_out)
          when ".pfx"
            cert_out = powershell_exec("openssl pkcs12 -export -nokeys -in #{cert_obj.to_pem} -outform PFX").result
            out_file.puts(cert_out)
          when ".p7b"
            cert_out = powershell_exec("openssl pkcs7 -export -nokeys -in #{cert_obj.to_pem} -outform P7B").result
            out_file.puts(cert_out)
          else
  ...
  else
  {
    ...
    $fullpath = "$Env:ProgramData\\Microsoft\\Crypto\\RSA\\$userSID\\$keyname"
        ...
        #
        def binary_cert?
          powershell_exec!("file -b --mime-encoding #{new_resource.source}").result.strip == "binary"
        end
